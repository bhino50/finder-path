...
The entire app lives in one file: `FinderPath/FinderPathApp.swift` (~925 lines). When changing behavior, expect to edit that single file rather than navigating a multi-module layout.
...
- `TerminalBridge.open(at:)` uses `NSWorkspace` to open a folder in Terminal. `TerminalBridge.openAgent(...)` uses Terminal's AppleScript `do script` because running a command in a new Terminal session can't be done through `NSWorkspace`. Both escape shell and AppleScript strings carefully — use `ShellCommand.argument(_:quoteStyle:)` and the local `appleScriptString(_:)` helper rather than building strings ad hoc.
